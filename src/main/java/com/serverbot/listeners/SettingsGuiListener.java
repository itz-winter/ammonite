...
public class SettingsGuiListener extends ListenerAdapter {
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (!event.isFromGuild() || !PermissionManager.hasPermission(event.getMember(), "admin.settings")) {
            event.reply(CustomEmojis.ERROR + " You no longer have permission to configure settings.").setEphemeral(true)
                    .queue();
            return;
        }
        ...
        switch (action) {
            ...
            case "automod" -> {
                toggle(guildId, "automodEnabled", settings);
                refreshPanel(event, guildId, guild, userId);
            }
            case "pronouns" -> {
                toggle(guildId, "pronounsEnabled", settings);
                refreshPanel(event, guildId, guild, userId);
            }
            case "dm-notify" -> {
                toggle(guildId, "dmNotificationsEnabled", settings);
                refreshPanel(event, guildId, guild, userId);
            }

            case "daily-reward" ->
                event.replyModal(intModal(userId, "sgm:daily-reward", "Set Daily Reward", "Coins (1\u20131,000,000)", 1,
                        1_000_000, SettingsCommand.getInt(settings, "dailyReward", 500))).queue();
            case "work-reward" ->
            ...
            case "points-per-msg" -> event
                    ...
                    .queue();
            case "xp-per-msg" -> event.replyModal(intModal(userId, "sgm:xp-per-msg", "Set XP per Message",
                    "XP (0\u201310000)", 0, 10000, SettingsCommand.getInt(settings, "xpPerMessage", 10))).queue();
            case "max-warnings" -> event.replyModal(intModal(userId, "sgm:max-warnings", "Set Max Warnings",
                    "Warnings (1\u201350)", 1, 50, SettingsCommand.getInt(settings, "maxWarnings", 3))).queue();
        ...
        switch (action) {
            ...
            case "mute-role" -> {
                ...
                if (role == null) {
                    ...
                    role = guild.getRoles().stream().filter(r -> r.getName().equalsIgnoreCase(lower)).findFirst()
                            .orElse(null);
                }
                if (role == null) {
                    event.reply(
