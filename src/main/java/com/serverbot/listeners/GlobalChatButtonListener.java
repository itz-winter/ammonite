 ...
 * All management actions use modal forms — no conversational follow-up
 * messages.
 *
...
public class GlobalChatButtonListener extends ListenerAdapter {
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (gc == null) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Not Found", "Channel not found.")).setEphemeral(true)
                    .queue();
            return;
        }
        ...
        switch (action) {
            ...
            case "gc_kick" -> {
                if (!gc.hasModerateAccess(userId)) {
                    noAccess(event);
                    return;
                }
            ...
            case "gc_delete" -> {
                ...
                if (gc.isCoOwner(userId) && !gc.isOwner(userId)) {
                    event.getJDA().retrieveUserById(gc.getOwnerId())
                            .queue(owner -> owner.openPrivateChannel().queue(dm -> dm
                    ...
                    event.reply(
                            CustomEmojis.SUCCESS + " Global chat channel **" + gc.getName() + "** has been deleted.")
                            .queue();
                }
            }
            ...
            case "gc_confirm_delete" -> {
                ...
                service.deleteChannel(channelId);
                event.reply(CustomEmojis.SUCCESS + " Global chat channel **" + gc.getName() + "** has been deleted.")
                        .queue();
            }
    ...
    public void onModalInteraction(ModalInteractionEvent event) {
        ...
        switch (action) {
            ...
            case "edit" -> {
                ...
                if (webhookmode.equals("webhook"))
                    gc.setWebhookEnabled(true);
                else if (webhookmode.equals("text"))
            ...
            case "display" -> {
                ...
                if (!prefix.isEmpty()) {
                    if ("reset".equalsIgnoreCase(prefix))
                        gc.setMsgPrefix(null);
                    else
                        gc.setMsgPrefix("{}".equals(prefix) ? "" : prefix);
                }
                if (!suffix.isEmpty()) {
                    if ("reset".equalsIgnoreCase(suffix))
                        gc.setMsgSuffix(null);
                    else
                        gc.setMsgSuffix("{}".equals(suffix) ? "" : suffix);
                }
            ...
            case "addmod" -> {
                ...
                if (error != null) {
                    event.reply(CustomEmojis.ERROR + " " + error).queue();
                    return;
                }
            ...
            case "mute" -> {
                ...
                event.reply(error != null ? CustomEmojis.ERROR + " " + error
                        : CustomEmojis.SUCCESS + " Server `" + serverId + "` muted"
                                + (durationMs <= 0 ? " permanently." : " for " + durationStr + "."))
                        .queue();
            }
    ...
    private Modal buildEditModal(GlobalChatChannel gc) {
        ...
        if (gc.getDescription() != null)
            desc.setValue(gc.getDescription());
    ...
    private Modal buildDisplayModal(GlobalChatChannel gc) {
        ...
        if (nf != null)
            nameformat.setValue(nf.isEmpty() ? "{}" : nf);
    ...
    private Modal buildSetRulesModal(GlobalChatChannel gc) {
        ...
        if (!gc.getRules().isEmpty())
            rules.setValue(String.join(" | ", gc.getRules()));
