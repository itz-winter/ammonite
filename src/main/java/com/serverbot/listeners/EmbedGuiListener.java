...
public class EmbedGuiListener extends ListenerAdapter {
    ...
    public void onModalInteraction(ModalInteractionEvent event) {
        ...
        switch (action) {
            ...
            case "field" -> {
                ...
                boolean inline = "yes".equalsIgnoreCase(inlineS) || "true".equalsIgnoreCase(inlineS)
                        || "y".equalsIgnoreCase(inlineS);
        ...
        // Acknowledge the modal (ephemeral, instantly deleted) then update the GUI via
        // the stored hook.
        event.deferReply(true).queue(hook -> hook.deleteOriginal().queue());

        if (session.hook != null) {
            TextChannel targetCh = event.isFromGuild()
                    ? event.getGuild().getTextChannelById(session.targetChannelId)
                    : null;
            session.hook.editOriginal(EmbedGuiCommand.buildGuiEdit(session, targetCh, userId)).queue();
        }
    }
    ...
    private void refreshGui(ButtonInteractionEvent event, EmbedGuiSession session, String userId) {
        TextChannel targetCh = event.isFromGuild()
                ? event.getGuild().getTextChannelById(session.targetChannelId)
                : null;
        event.editMessage(EmbedGuiCommand.buildGuiEdit(session, targetCh, userId)).queue();
    }
    ...
    private void handleSend(ButtonInteractionEvent event, EmbedGuiSession session, String userId) {
        // Double-check permission at send time (session may have been created before
        // perm change)
        if (!session.canSend) {
        ...
        sendAction.queue(
                msg -> {
                    ...
                    // Replace the single GUI message with a success embed
    ...
    private Modal authorModal(String uid, EmbedGuiSession s) {
        ...
        if (s.authorIconUrl != null)
            icon.setValue(s.authorIconUrl);
        TextInput.Builder url = TextInput.create("url", TextInputStyle.SHORT)
                .setPlaceholder("Author link URL (leave blank for none)")
                .setMaxLength(2048).setRequired(false);
        if (s.authorUrl != null)
            url.setValue(s.authorUrl);
        return Modal.create("egm:author:" + uid, "Set Author")
                .addComponents(Label.of("Author Name", name.build()), Label.of("Icon URL (optional)", icon.build()),
                        Label.of("Link URL (optional)", url.build()))
                .build();
    }
    ...
    private Modal fieldModal(String uid) {
        ...
        return Modal.create("egm:field:" + uid, "Add Field")
                .addComponents(Label.of("Field Name", name), Label.of("Field Value", value),
                        Label.of("Inline? (yes/no)", inline))
                .build();
    }
