    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (targetUser.isBot()) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Invalid Target",
                    "Invalid target!")).setEphemeral(true).setComponents(net.dv8tion.jda.api.components.actionrow.ActionRow.of(net.dv8tion.jda.api.components.buttons.Button.secondary("share_req:" + event.getUser().getId(), "\uD83D\uDCE4 Share"))).queue();
            return;
        }
