...
public class PresenceCommand implements SlashCommand {
    ...
    private void setPresence(SlashCommandInteractionEvent event) {
        String presenceText = event.getOption("text") != null ? event.getOption("text").getAsString() : null;
        String activityType = event.getOption("type") != null ? event.getOption("type").getAsString() : "playing";
        ...
        try {
            Activity activity = switch (activityType.toLowerCase()) {
                ...
                case "streaming" -> Activity.streaming(presenceText, "https://twitch.tv/placeholder"); // for some
                                                                                                       // fuckass reason
                                                                                                       // the url is
                                                                                                       // required??
                                                                                                       // DONT TOUCH IT
