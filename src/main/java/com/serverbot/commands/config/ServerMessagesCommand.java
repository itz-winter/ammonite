...
/**
 * /servermessages — configure per-server message pools for various bot responses.
 *
...
public class ServerMessagesCommand implements SlashCommand {
    ...
    @Override
    public String getName() { return "servermessages"; }

    @Override
    public String getDescription() { return "Configure per-server message pools for bot responses (work, daily, etc.)"; }
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (!PermissionManager.hasPermission(member, "admin.settings")) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Insufficient Permissions", "You need the `admin.settings` permission.")).setEphemeral(true).queue();
            return;
        }

        String sub = event.getSubcommandName();
        if (sub == null) { showHelp(event); return; }
        switch (sub) {
    ...
    private void handleList(SlashCommandInteractionEvent event) {
        ...
        if (!VALID_TYPES.contains(type)) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Invalid Type", "Valid types: `" + TYPE_LIST + "`")).setEphemeral(true).queue();
            return;
        }

        List<String> msgs = ServerBot.getStorageManager().getCustomGuildMessages(guildId, type);
        if (msgs == null || msgs.isEmpty()) {
            event.replyEmbeds(EmbedUtils.createInfoEmbed("Default Messages", "No custom messages set for `" + type + "`. Using the built-in defaults.\nUse `/servermessages add` to add custom ones.")).setEphemeral(true).queue();
            return;
        }
    ...
    private void handleAdd(SlashCommandInteractionEvent event) {
        ...
        if (type == null || message == null || message.isBlank()) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Missing Options", "Please provide both type and message.")).setEphemeral(true).queue();
            return;
        }
        if (!VALID_TYPES.contains(type)) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Invalid Type", "Valid types: `" + TYPE_LIST + "`")).setEphemeral(true).queue();
        ...
        if (message.length() > 500) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Too Long", "Messages must be 500 characters or less.")).setEphemeral(true).queue();
            return;
        }
    ...
    private void handleRemove(SlashCommandInteractionEvent event) {
        ...
        if (type == null || pos == null) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Missing Options", "Please provide type and position.")).setEphemeral(true).queue();
            return;
        }
        if (!VALID_TYPES.contains(type)) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Invalid Type", "Valid types: `" + TYPE_LIST + "`")).setEphemeral(true).queue();
        ...
        if (!removed) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Invalid Position", "No message at position " + pos + " for `" + type + "`.")).setEphemeral(true).queue();
            return;
        }
        DismissibleMessage.reply(event, new EmbedBuilder()
            .setColor(EmbedUtils.SUCCESS_COLOR)
    ...
    private void handleReset(SlashCommandInteractionEvent event) {
        ...
        if (type == null) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Missing Type", "Please specify the message type to reset.")).setEphemeral(true).queue();
            return;
        }
        if (!VALID_TYPES.contains(type)) {
            event.replyEmbeds(EmbedUtils.createErrorEmbed("Invalid Type", "Valid types: `" + TYPE_LIST + "`")).setEphemeral(true).queue();
        ...
        DismissibleMessage.reply(event, new EmbedBuilder()
            ...
            .setDescription("Custom messages for `" + type + "` have been removed. The bot will now use the built-in defaults.")
            .build(), true);
    }
    ...
    private void showHelp(SlashCommandInteractionEvent event) {
        DismissibleMessage.reply(event, new EmbedBuilder()
            ...
            .addField("Commands",
                ...
                "`/servermessages reset <type>` — restore defaults", false)
            .build(), true);
    }
