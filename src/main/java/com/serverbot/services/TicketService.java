...
public class TicketService {
                ...
                if (jda != null) {
                    ...
                    for (net.dv8tion.jda.api.entities.Guild guild : jda.getGuilds()) {
                        try {
                            ...
                            ServerBot.getLogger().error(
                                    "Error cleaning up tickets for guild " + guild.getName() + ": " + e.getMessage(),
                                    e);
                        }
                    }
                }
    ...
    public CompletableFuture<String> addUserToTicket(Guild guild, String ticketId, User userToAdd) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                channel.getManager()
                        .putMemberPermissionOverride(userToAdd.getIdLong(),
                                EnumSet.of(Permission.VIEW_CHANNEL, Permission.MESSAGE_SEND,
                                        Permission.MESSAGE_HISTORY),
                                null)
                        .queue();
            ...
            try {
                ...
                channel.sendMessageEmbeds(new EmbedBuilder()
                        ...
                        .addComponents(ActionRow.of(
                                Button.primary("ticket_archive", "Archive Ticket").withEmoji(Emoji.fromUnicode("📦"))))
        ...
        try {
            ...
            if (file.exists() && file.length() > 0) {
                try (FileReader reader = new FileReader(file)) {
                    Type type = new TypeToken<Map<String, TicketCategory>>() {
                    }.getType();
    ...
    private void loadTicketData() {
        try {
            ...
            if (file.exists() && file.length() > 0) {
                try (FileReader reader = new FileReader(file)) {
                    Type type = new TypeToken<Map<String, TicketData>>() {
                    }.getType();
    ...
    private void loadTicketSettings() {
        try {
            ...
            if (file.exists() && file.length() > 0) {
                try (FileReader reader = new FileReader(file)) {
                    Type type = new TypeToken<Map<String, TicketSettings>>() {
                    }.getType();
    ...
    public static class TicketCategory {
        ...
        public TicketCategory(String categoryId, String name, String description, String emoji,
    ...
    public static class TicketSettings {
        ...
        public boolean isDmTranscripts() {
            return dmTranscripts;
        }

        public void setDmTranscripts(boolean dmTranscripts) {
            this.dmTranscripts = dmTranscripts;
        }

        public boolean isCloseOnLeave() {
            return closeOnLeave;
        }

        public void setCloseOnLeave(boolean closeOnLeave) {
        ...
        public void setSupportRoles(List<String> supportRoles) {
     ...
     */
    public CompletableFuture<String> createTicketKiosk(Guild guild, TextChannel channel, String title,
            String description) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                EmbedBuilder embed = new EmbedBuilder()
    ...
    /**
     * Get all closed tickets that are older than the deletion delay and not
     * archived
     */
    ...
    private void loadTicketKiosks() {
        try {
            ...
            if (file.exists() && file.length() > 0) {
                try (FileReader reader = new FileReader(file)) {
                    Type type = new TypeToken<Map<String, TicketKiosk>>() {
                    }.getType();
